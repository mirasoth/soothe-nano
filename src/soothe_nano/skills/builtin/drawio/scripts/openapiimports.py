...
def load_spec(path):
    ...
    if path.lower().endswith((".yaml", ".yml")):
        try:
            ...
            sys.exit(
                "error: spec is YAML but PyYAML is not installed "
...
def method_style(method):
    ...
    return (
        "rounded=1;whiteSpace=wrap;html=1;align=left;spacingLeft=6;"
...
def main():
    ...
    ap.add_argument(
        "--no-schemas", action="store_true", help="show only endpoints, omit schema nodes and edges"
    )
    args = ap.parse_args()
    ...
    sys.stderr.write(
        f"{ops} operations, {len(graph['nodes']) - ops} schemas, {len(graph['edges'])} edges\n"
    )
