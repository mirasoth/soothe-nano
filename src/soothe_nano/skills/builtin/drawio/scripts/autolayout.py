...
def build_dot(graph):
    ...
    lines = [
        f"digraph G {{ rankdir={rankdir};{sep} splines=ortho; node [shape=box fixedsize=true];"
    ]
...
def page_cells(graph, height, pos, edge_pts, color=True):
    ...
    for p in sorted(ordered, key=len, reverse=True):
        xs = [
            (rects[m][0], rects[m][1], rects[m][0] + rects[m][2], rects[m][1] + rects[m][3])
    ...
    for node in nodes:
        ...
        if node.get("link"):
            ...
            cells.append(
                f'        <mxCell id="{attr(nid)}" value="{attr(node.get("label", nid))}" ' + body
            )
...
def route_score(graph, height, pos, edge_pts):
    ...
    for edge in graph.get("edges", []):
        ...
        if pts:
            routes.append(
                ([(x * 72, (height - y) * 72) for x, y in pts], {edge["source"], edge["target"]})
            )
    through = sum(
        1
        for pts, ends in routes
        for nid, box in rects.items()
    ...
    length = sum(
        abs(b[0] - a[0]) + abs(b[1] - a[1]) for pts, _ in routes for a, b in zip(pts, pts[1:])
    )
...
def main():
    ...
    ap.add_argument(
        "--mono", action="store_true", help="don't colour groups by palette (monochrome boxes)"
    )
    ap.add_argument(
        "--tune",
        action="store_true",
