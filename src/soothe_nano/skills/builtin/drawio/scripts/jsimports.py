...
def resolve(spec, importer, root, modules):
    ...
    candidates = [base + e for e in EXTS] + [os.path.join(base, "index" + e) for e in EXTS] + [base]
...
def main():
    ...
    ap.add_argument(
        "--group", action="store_true", help="group nodes into containers by top-level directory"
    )
    ap.add_argument(
    ...
    edges = sorted({(m, t) for m, p in modules.items() for t in edges_of(m, p, root, modules)})
    raw = len(edges)
    ...
    def label(m):
        return (m[len(strip) :] if strip and m.startswith(strip) else m) or m
