...
def discover(root, modpath):
    ...
    for dirpath, dirs, files in os.walk(root):
        dirs[:] = [d for d in dirs if d not in ("vendor", "testdata") and not d.startswith(".")]
        gofiles = [
...
def main():
    ...
    ap.add_argument(
        "--group", action="store_true", help="group nodes into containers by top-level package dir"
    )
    ap.add_argument(
    ...
    edges = sorted(
        {(ip, t) for ip, files in pkgs.items() for t in imports_of(files, modpath, pkgs) if t != ip}
    )
    raw = len(edges)
    ...
    def label(ip):
        return ip[len(strip) :] if ip.startswith(strip) else os.path.basename(ip)
