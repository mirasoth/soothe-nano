...
def pages_of(path):
    ...
    return [
        (d.get("id") or f"p{i}", d.get("name") or f"Page {i + 1}")
...
def export_svg(drawio_file, index, out_svg):
    ...
    r = subprocess.run(
        ["drawio", "-x", "-f", "svg", "--page-index", str(index), "-o", out_svg, drawio_file],
        capture_output=True,
    )
...
def build_html(title, page_meta, svgs):
    ...
    tabs = json.dumps([{"id": pid, "name": name} for pid, name in page_meta]).replace("</", "<\\/")
...
def main():
    ap = argparse.ArgumentParser(
        description="Export a .drawio to a self-contained interactive HTML viewer."
    )
    ...
    sys.stderr.write(
        f"wrote {out} ({len(svgs)} page{'s' if len(svgs) != 1 else ''}"
