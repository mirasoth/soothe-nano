...
def build(data, grid=110):
    ...
    for ln in lines:
        for sid in ln.get("stations", []):
            if sid not in stations:
                sys.exit(
                    f"error: line {ln.get('name', '?')!r} references unknown station id {sid!r}"
                )
    ...
    def px(sid):
        s = stations[sid]
        return ox + int(s["gx"]) * grid_step, oy + int(s["gy"]) * grid_step

    maxx = ox + max(int(s["gx"]) for s in stations.values()) * grid_step + 220
    maxy = oy + max(int(s["gy"]) for s in stations.values()) * grid_step + 120
    lw = max(8, grid_step // 12)
