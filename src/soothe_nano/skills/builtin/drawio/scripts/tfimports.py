...
class IconResolver:
    ...
    def resolve(self, rtype):
        ...
        if provider in PROVIDERS:
            ...
            words = (
                QUERY_OVERRIDES.get(rtype) or f"{prefix} {rtype.split('_', 1)[1].replace('_', ' ')}"
            ).split()
            ...
            while len(words) > 1 and hit is None:
                ...
                good = [
                    r
                    for r in self.ss.search(self.shapes, self.tag_map, " ".join(words), 40)
                    if r["style"] in allowed
                    and want(r["style"])
...
def transitive_reduce(nodes, edges):
    ...
    try:
        out = subprocess.run(["tred"], input=dot, capture_output=True, text=True, check=True).stdout
...
def main():
    ...
    ap.add_argument(
        "--no-reduce", action="store_true", help="keep every edge (skip transitive reduction)"
    )
    ap.add_argument(
        "--no-icons", action="store_true", help="plain boxes instead of official cloud icons"
    )
    args = ap.parse_args()
    ...
    edges = sorted(
        {
            (nid, ref)
            for nid, (_, _, body) in declared.items()
            for ref in _REF.findall(body)
            if ref in declared and ref != nid
        }
    )
    raw = len(edges)
