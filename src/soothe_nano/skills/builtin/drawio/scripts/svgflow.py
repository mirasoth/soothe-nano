...
def to_svg(path):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = subprocess.run(["drawio", "-x", "-f", "svg", "-o", out, path], capture_output=True)
...
def animate(svg, speed, dash, reverse):
    ...
    style = (
        f"<style>.dio-flow{{stroke-dasharray:{dash};"
...
def main():
    ...
    if n == 0:
        sys.stderr.write("warning: no edges found to animate (a diagram with no connectors?)\n")
