...
def main():
    ...
    def keyed(nodes):
        ...
        if args.by_label:
            return {lbl: lbl for lbl, _ in nodes.values()}, {
                i: lbl for i, (lbl, _) in nodes.items()
            }
    ...
    for key in sorted(set(old_keys) | set(new_keys)):
        ...
        counts[status] += 1
        nodes.append(
            {"id": key, "label": label or key, "style": STYLE[status], "width": 160, "height": 60}
        )
    ...
    for s, t in sorted(old_ek | new_ek):
        ...
        status = (
            ...
            else ("added" if (s, t) in new_ek else "removed")
        )
