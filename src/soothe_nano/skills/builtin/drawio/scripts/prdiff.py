...
def changed_drawios(base, head, repo):
    ...
    for line in r.stdout.splitlines():
        ...
        status = (
            "added" if code.startswith("A") else "removed" if code.startswith("D") else "modified"
        )
...
def export_png(src_drawio, out_png):
    ...
    r = subprocess.run(
        ["drawio", "-x", "-f", "png", "--page-index", "1", "-o", out_png, src_drawio],
        capture_output=True,
    )
...
def export_diff_png(base_drawio, head_drawio, out_png, tmp):
    ...
    r1 = subprocess.run(
        [
            sys.executable,
            os.path.join(HERE, "drawiodiff.py"),
            base_drawio,
            head_drawio,
            "-o",
            diff_json,
    ...
    if r1.returncode != 0 or not os.path.exists(diff_json):
        return False
    r2 = subprocess.run(
        [sys.executable, os.path.join(HERE, "autolayout.py"), diff_json, "-o", diff_drawio],
        capture_output=True,
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Render PNGs + a Markdown report for .drawio files "
    ...
    ap.add_argument(
        "--out-dir", default="drawio-pr", help="directory for exported PNGs (default: ./drawio-pr)"
    )
    ...
    entries = [
        build_entry(args.repo, args.base, args.head, path, status, args.out_dir, drawio_available)
