        ...
        cells.append(
            {
                "id": el.get("id"),
                "el": el,
                "vertex": inner.get("vertex") == "1",
                "edge": inner.get("edge") == "1",
...
def main():
    ap = argparse.ArgumentParser(
        description="Animate a .drawio building itself -> self-contained HTML player."
    )
    ...
    ap.add_argument(
        "--keep-frames", action="store_true", help="also write the PNG frames next to the output"
    )
    args = ap.parse_args()
