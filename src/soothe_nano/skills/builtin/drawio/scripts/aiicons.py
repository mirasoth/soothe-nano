...
def main():
    ...
    ap.add_argument(
        "--size", type=int, default=48, help="cell width/height in px (icons are square)"
    )
    ap.add_argument(
        "--embed",
        action="store_true",
    ...
    if matches:
        for base in matches:
            ...
            if args.embed:
                ...
                svg = svg.replace(b'width="1em"', b'width="24"').replace(
                    b'height="1em"', b'height="24"'
                )
    ...
    if not results:
        sys.exit(
            f"no logo for {args.query!r} — for a data store try a cylinder "
