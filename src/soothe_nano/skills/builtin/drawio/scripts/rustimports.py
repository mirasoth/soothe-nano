...
def crate_name(root):
    ...
    if os.path.exists(cargo):
        m = re.search(
            r'(?m)^\s*name\s*=\s*"([^"]+)"', open(cargo, encoding="utf-8", errors="ignore").read()
        )
        if m:
            return m.group(1)
    return "crate"
...
def main():
    ...
    ap.add_argument(
        "--group", action="store_true", help="box modules by their parent module path (nested)"
    )
    ap.add_argument(
    ...
    edges = sorted(
        {(mid(m), mid(t)) for m, path in modules.items() for t in edges_of(m, path, modules)}
    )
    raw = len(edges)
