...
def main():
    ...
    for c in containers:
        ...
        attached = [
            n
            for n in ((c.get("NetworkSettings") or {}).get("Networks") or {})
    ...
    for net in sorted(nets):
        nodes.append(
            {"id": f"net:{net}", "label": net, "style": NETWORK_STYLE, "width": 120, "height": 70}
        )
    for vol in sorted(vols):
        nodes.append(
            {"id": f"vol:{vol}", "label": vol, "style": VOLUME_STYLE, "width": 120, "height": 70}
        )
    ...
    sys.stderr.write(
        f"{len(nodes)} nodes ({len(containers)} containers, "
