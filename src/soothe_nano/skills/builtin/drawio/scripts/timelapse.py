...
def build_frame(importer, importer_args, work_path, direction, tmp):
    ...
    exp = subprocess.run(
        ["drawio", "-x", "-f", "png", "--width", "1600", "-o", png, drawio], capture_output=True
    )
...
def main():
    ...
    ap.add_argument(
        "--importer-args", default="", help='extra args passed to the importer, e.g. "--group"'
    )
    ...
    ap.add_argument(
        "--keep-frames", action="store_true", help="also write the PNG frames next to the HTML"
    )
    args = ap.parse_args()
    ...
    if importer not in IMPORTERS:
        sys.exit(
            f"error: unknown importer {importer!r} (choose one of: "
