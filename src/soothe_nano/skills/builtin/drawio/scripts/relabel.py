...
def label_slots(tree):
    ...
    for diagram in tree.getroot().iter("diagram"):
        ...
        if root is None:  # compressed page — can't edit
            sys.stderr.write(
                "warning: skipping compressed page "
...
def main():
    ...
    mode.add_argument("--extract", action="store_true", help="dump an identity label map as JSON")
    mode.add_argument(
        "--map", metavar="JSON", dest="mapfile", help="JSON map {old label: new label} to apply"
    )
    p.add_argument(
        "-o",
        "--output",
        help="output path (default: stdout for --extract, <file>-relabel.drawio for --map)",
    )
    args = p.parse_args()
    ...
    if unused:
        sys.stderr.write(
            f"warning: {len(unused):d} map key(s) matched no label: "
            f"{', '.join(repr(k)[:60] for k in unused[:10])}\n"
        )
