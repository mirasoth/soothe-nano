...
def main():
    ...
    if args.clusters:
        sys.stderr.write(
            "note: --clusters is a soft hint; label propagation "
    ...
    exec_nodes = [
        {"id": f"c_{c}", "label": names[c], "link": f"data:page/id,{page2_id}"} for c in communities
    ]
    exec_edges = [
        {"source": f"c_{s}", "target": f"c_{t}", "label": str(n) if n > 1 else ""}
    ...
    if args.output:
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(xml)
        sys.stderr.write(
            f"wrote {args.output} ({len(nodes)} nodes -> {len(communities)} clusters)\n"
        )
    else:
