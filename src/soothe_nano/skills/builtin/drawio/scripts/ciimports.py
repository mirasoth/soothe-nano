...
def find_workflows(path):
    ...
    files = (
        sorted(os.path.join(wfdir, f) for f in os.listdir(wfdir) if f.endswith((".yml", ".yaml")))
...
def parse_actions(spec, wf_id, wf_name, group):
    ...
    events = (
        sorted(on) if isinstance(on, dict) else ([on] if isinstance(on, str) else sorted(on or []))
    )
    trig_id = f"{wf_id}//trigger"
    nodes.append(
        {
            "id": trig_id,
            "label": "on: " + (", ".join(events) or "?"),
            "style": TRIGGER_STYLE,
            "width": 160,
            "height": 50,
            "group": group,
...
def parse_gitlab(spec, wf_id, group_prefix):
    ...
    for jid, job in jobs.items():
        ...
        if needs:
            ...
            if i > 0:
                edges.extend(
                    {"source": f"{wf_id}//{p}", "target": f"{wf_id}//{jid}"}
...
def main():
    ...
    for path in files:
        ...
        if os.path.basename(path) == ".gitlab-ci.yml" or ("jobs" not in spec and "stages" in spec):
            n, e = parse_gitlab(spec, wf_id, "stage: " if len(files) == 1 else f"{wf_id} / stage: ")
        elif spec.get("jobs"):
