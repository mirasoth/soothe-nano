...
def hue_slot(hexcolor, palette):
    ...
    if sat < 0.15 or light > 0.97 or light < 0.03:  # grey / near-white / near-black
        slot = "neutral"
    else:
        deg = hue * 360
        slot = min(
            SLOT_HUES, key=lambda k: min(abs(deg - SLOT_HUES[k]), 360 - abs(deg - SLOT_HUES[k]))
        )
    if palette.get(slot):
        return slot
...
def main():
    ...
    if extras.get("globalStrokeWidth") not in (None, 1):
        vertex_extra["strokeWidth"] = f"{extras['globalStrokeWidth']:g}"
        ...
        for child in root:
            ...
            if cell.get("edge") == "1":
                ...
                if extras.get("edgeColor"):
                    ...
                    kv.update(
                        strokeColor=extras["edgeColor"],
                        fontColor=extras["edgeColor"],
                        labelBackgroundColor="none",
                    )
                if extras.get("sketch"):
                    kv["sketch"] = "1"
                if extras.get("globalStrokeWidth") not in (None, 1):
                    kv["strokeWidth"] = f"{extras['globalStrokeWidth']:g}"
                if kv:
    ...
    out = args.output or f"{os.path.splitext(args.file)[0]}-{preset.get('name', 'restyled')}.drawio"
    tree.write(out, encoding="utf-8", xml_declaration=False)
    remap = ", ".join(f"{c}->{s}" for c, s in sorted(slot_map.items()))
    sys.stderr.write(
        f"wrote {out} ({n_vert} vertices, {n_edge} edges restyled"
