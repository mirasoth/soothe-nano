...
ER_EDGE = (
    "edgeStyle=orthogonalEdgeStyle;html=1;rounded=0;fontSize=11;"
    "labelBackgroundColor=#ffffff;"
    "startArrow=ERmany;startFill=0;endArrow=ERone;endFill=0;"
)

_COMMENT = re.compile(r"/\*.*?\*/|--[^\n]*", re.S)
_CREATE = re.compile(r"\bCREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([\w.\"`\[\]]+)\s*\(", re.I)
_FK = re.compile(
...
_SKIP = re.compile(r"^\s*(CONSTRAINT|UNIQUE|CHECK|KEY|INDEX|FULLTEXT|SPATIAL|EXCLUDE|LIKE)\b", re.I)
...
def main():
    ...
    ap.add_argument(
        "--no-types", action="store_true", help="list column names only (hide the SQL types)"
    )
    args = ap.parse_args()
    ...
    for name, t in tables.items():
        ...
        width = max(160, -(-max(7 * len(line) + 30 for line in lines) // 10) * 10)
        ...
        for col, ref in t["fks"]:
            if ref in tables and ref != name:
                edges.append({"source": name, "target": ref, "label": col, "style": ER_EDGE})
