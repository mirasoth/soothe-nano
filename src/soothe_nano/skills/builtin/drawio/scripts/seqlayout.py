...
ACTOR = LIFELINE + (
    "participant=umlActor;verticalAlign=bottom;spacingBottom=-14;labelBackgroundColor=#ffffff;"
)
...
def main():
    ...
    if args.output:
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(xml)
        print(
            f"wrote {args.output} ({len(spec['participants'])} participants, "
            f"{len(spec.get('messages', []))} messages)",
            file=sys.stderr,
        )
    else:
