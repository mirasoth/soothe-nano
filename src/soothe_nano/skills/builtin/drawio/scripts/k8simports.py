...
WORKLOADS = {"Deployment", "StatefulSet", "DaemonSet", "ReplicaSet", "Job", "CronJob", "Pod"}
_INDEX = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "data", "shape-index.json.gz"
)
...
def main():
    ...
    ap.add_argument(
        "--no-icons", action="store_true", help="plain boxes instead of official Kubernetes icons"
    )
    args = ap.parse_args()
