...
def endpoint(edge, end, by_id):
    ...
    return (x + (fx if fx is not None else 0.5) * w, y + (fy if fy is not None else 0.5) * h)
...
def geometry_warnings(cells, ids, parents):
    ...
    for c in cells:
        if c.get("edge") == "1":
            ...
            if pts:
                routed.append((c.get("id"), pts, {c.get("source"), c.get("target")}))
    ...
    leaves = [
        (c.get("id"), abs_rect(c, ids))
        for c in cells
        if c.get("vertex") == "1" and c.get("id") not in parents and not is_edge_label(c)
    ]
...
def main():
    ...
    if args.score:
        ...
        print(
            f"score: {20 * through + 10 * cross + 5 * olap} "
